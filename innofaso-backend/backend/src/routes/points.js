...
router.post("/", auth, requireAdmin, async (req, res) => {
  const { id, zone_map_id, label, x, y, point_type, description } = req.body;
  ...
  try {
    await db.query(
      ...
      [id.trim(), zone_map_id, label.trim(), Number(x), Number(y), point_type || "1", description || ""]
    );
    res.status(201).json({ id: id.trim(), zone_map_id, label: label.trim(), x: Number(x), y: Number(y), point_type: point_type || "1", description: description || "" });
  } catch (err) {
...
router.put("/:id", auth, requireAdmin, async (req, res) => {
  const { zone_map_id, label, x, y, point_type, description } = req.body;
  try {
    const [result] = await db.query(
      "UPDATE sampling_points SET zone_map_id=?, label=?, x=?, y=?, point_type=?, description=? WHERE id=?",
      [zone_map_id, label, Number(x), Number(y), point_type || "1", description || "", req.params.id]
    );
    if (result.affectedRows === 0) return res.status(404).json({ error: "Point introuvable." });
    res.json({ id: req.params.id, zone_map_id, label, x: Number(x), y: Number(y), point_type: point_type || "1", description: description || "" });
  } catch (err) {
