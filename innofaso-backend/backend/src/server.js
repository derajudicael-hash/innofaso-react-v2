...
const DEFAULT_POINTS = [
  ...
  { id: '2.3.1a',   zone_map_id: 'huile',               label: '2.3.1',    x: 74.5, y: 30,   point_type: '2', description: 'Plateau balance pesée pré mélange',                   ufc: 24   },
  ...
  { id: '4.14.2a',  zone_map_id: 'vestiaires_h',        label: '4.14.2',   x: 51.5, y: 82.5, point_type: '4', description: 'Poignet vestiaire homme',                              ufc: 110  },
  ...
  { id: '4.16.3a',  zone_map_id: 'vestiaires_visiteur', label: '4.16.3',   x: 58.5, y: 82.5, point_type: '4', description: 'Sols de vestiaire visiteur',                           ufc: 220  },
...
(async () => {
  try {
    ...
    if (cnt === 0) {
      ...
      // Migration : applique les valeurs UFC de démo sur les points encore à NULL
      for (const p of DEFAULT_POINTS) {
        if (p.ufc !== undefined && p.ufc !== null) {
          await db.query(
            "UPDATE sampling_points SET ufc = ? WHERE id = ? AND ufc IS NULL",
            [p.ufc, p.id]
          );
        }
      }
